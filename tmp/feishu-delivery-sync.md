...
> 说明：本文档是面向飞书同步和最终提交的精简交付版；如需仓库内保留的长版说明，请看 项目交付文档初稿.md（docs/项目交付文档初稿.md）。
...
为了让交付文档更接近最终验收材料，而不是只有技术说明，当前已补充和功能模块一一对应的演示截图。建议答辩或提交时按“功能说明 + 对应截图”一起展示，覆盖范围如下：

| 模块 | 已补充截图 | 对应证明点 |
| --- | --- | --- |
| 客户端首页与播放 | `主界面.jpg`、`播放界面.jpg` | 短剧列表、播放页和基础观看主链路已完成 |
| 低强度高光互动 | `笑点浮窗弹幕.jpg`、`反转浮窗弹幕.jpg` | 快捷弹幕 / 轻量互动层已落地 |
| 高强度高光互动 | `高光反转（未点击）.jpg`、`反转高光（点击）.jpg`、`爽点高光（点击）.jpg`、`温情高光（点击）.jpg`、`燃点高光（点击）.jpg`、`笑点高光（点击）.jpg` | 正式高光组件可按不同剧情强度触发 |
| 尾集分支体验 | `分支选择.jpg`、`分支展示头.jpg`、`分支展示尾.jpg` | 尾集入口、固定分支和图文分镜结果展示已完成 |
| 管理后台 | `管理面板总览.png`、`高光候选详情.png`、`分支任务后台.png` | 高光复核、后台总览和分支任务管理链路已完成 |
...
- Android APK：`docs/submission-assets/apk/drama-pulse-debug.apk`
- APK 说明：当前提供的是本地调试可安装包，归档路径为 `docs/submission-assets/apk/`
- APK SHA256：`edc3cac413d4bdc96430e66d14ff21db03a38251f164f202746c022cea9f6f66`
- 项目展示录屏：请补充到 `docs/submission-assets/videos/`，目录已预留
- 关键页面截图：已补充至 `docs/submission-assets/screenshots`
- APK 素材目录：`docs/submission-assets/apk`
- 截图素材目录：`docs/submission-assets/screenshots`
- 录屏素材目录：`docs/submission-assets/videos`
- 本交付主文档
- 技术方案与模块说明附录
- 本地启动说明
...
说明：用于展示项目已完成短剧列表页、封面浏览和基础内容入口。

**剧集播放界面**


说明：用于展示播放主链路、播放器控件和剧情消费场景。
...
说明：用于证明低强度高光采用轻量快捷弹幕，不打断观看流程。

**高强度高光组件触发前后**



说明：用于证明高强度高光会触发正式互动组件，并具备点击反馈。

补充说明：`爽点高光（点击）.jpg`、`温情高光（点击）.jpg`、`燃点高光（点击）.jpg`、`笑点高光（点击）.jpg` 进一步证明同一套高光数据链路已经支持多类剧情情绪。
...
说明：用于证明尾集结束后会弹出固定分支和自定义分支入口。

**图文分镜结果展示**



说明：用于证明当前版本不是单段文字续写，而是剧情正文与分镜内容交织展示的结果页。
...
说明：用于证明管理后台已具备整体运营与任务查看能力。

**高光候选详情**


说明：用于证明高光标签并非直接由 AI 实时驱动客户端，而是经过候选查看、人工复核和冻结后再下发。

**分支任务后台**


说明：用于证明尾集分支任务具备后台查看、追踪和重试能力。

## 14. 附录文档链接

- 技术选型与落地说明（docs/技术选型与落地说明.md）
- Android 端技术实现方案（docs/Android端技术实现方案.md）
- 本地服务端技术实现方案（docs/本地服务端技术实现方案.md）
- 高光识别与打标实现方案（docs/高光识别与打标实现方案.md）
- 尾集分支生成任务技术实施方案（docs/尾集分支生成任务技术实施方案.md）
- 本地启动与后台登录说明（docs/本地启动与后台登录说明.md）
